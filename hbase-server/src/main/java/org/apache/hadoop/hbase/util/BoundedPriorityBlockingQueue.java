...
public class BoundedPriorityBlockingQueue<E> extends AbstractQueue<E> implements BlockingQueue<E> {
  private static class PriorityQueue<E> {             //其内部实现了一个PriorityQueue
    ...
    public void add(E elem) {
      ...
      if (tail == head || comparator.compare(objects[tail - 1], elem) <= 0) {
        // Append
        objects[tail++] = elem;       //优先级队列插入队尾
      } else if (head > 0 && comparator.compare(objects[head], elem) > 0) {
        // Prepend
        objects[--head] = elem;       //优先级队列插入队首
      } else {
        // Insert in the middle
        int index = upperBound(head, tail - 1, elem);
        System.arraycopy(objects, index, objects, index + 1, tail - index);    //优先级队列插入中间位置
        objects[index] = elem;
        tail++;
      }
    }
    ...
    public E peek() {
      return (head != tail) ? objects[head] : null;   //返回队首元素
    }

    public E poll() {          //弹出队首元素(返回并删除队首)
      E elem = objects[head];
      objects[head] = null;
