...
/**
 * Implements the regionserver RPC services.   这个类实现了regionserver的RPC服务
 */
...
public class RSRpcServices implements HBaseRPCErrorHandler,
  ...
  final Counter requestCount = new Counter();       //请求计数;
  ...
  /**
   * Instantiated as a scanner lease. If the lease times out, the scanner is      //获得一个带租约的scan
   * closed      
   */
  ...
  private List<CellScannable> doNonAtomicRegionMutation(final Region region,
      ...
      try {
        ...
        if (r != null) {
          ...
          resultOrExceptionBuilder =
            ClientProtos.ResultOrException.newBuilder().setResult(pbResult);    //把result编译成Protobuf码并返回
        }
    ...
    // Finish up any outstanding mutations  下面一句进行的是批量操作
  ...
  private void doBatchOp(final RegionActionResult.Builder builder, final Region region,
    ...
    try {
      ...
      for (ClientProtos.Action action: mutations) {
        ...
        Mutation mutation;
        if (m.getMutateType() == MutationType.PUT) {    //这里是把put/delete操作分开处理的
          mutation = ProtobufUtil.toPut(m, cells);
          batchContainsPuts = true;
        } else {
          mutation = ProtobufUtil.toDelete(m, cells);   //应该是在mutation加了不同的标签
      ...
      OperationStatus codes[] = region.batchMutate(mArray, HConstants.NO_NONCE,
        HConstants.NO_NONCE);    //具体操作是由batchMutate方法来执行的,返回的OperationStatus数据记录了每个action的状态
  ...
  private OperationStatus [] doReplayBatchOp(final Region region,
    ...
    try {
      ...
      requestCount.add(mutations.size());               //返回的是put/delete的次数
      if (!region.getRegionInfo().isMetaTable()) {
        regionServer.cacheFlusher.reclaimMemStoreMemory();
      }
  ...
  public RSRpcServices(HRegionServer rs) throws IOException {
    ...
    rpcServer = new RpcServer(rs, name, getServices(),
      ...
      rs.conf,
      rpcSchedulerFactory.create(rs.conf, this, rs));     //new一个RpcServer产生一个rpc服务的实例返回
  ...
  public GetResponse get(final RpcController controller,
    ...
    try {
      ...
      if (get.hasClosestRowBefore() && get.getClosestRowBefore()) {
        ...
        if (existence == null) {
          r = region.get(clientGet);          //实际的get操作在这,r是返回的result型变量,结果
  ...
  throws ServiceException {
    ...
    PayloadCarryingRpcController controller = (PayloadCarryingRpcController)rpcc;  //PayloadCarryingRpcController属于后门,可以避免返回数据的序列化
    CellScanner cellScanner = controller != null ? controller.cellScanner(): null;
    if (controller != null) controller.setCellScanner(null);

    long nonceGroup = request.hasNonceGroup() ? request.getNonceGroup() : HConstants.NO_NONCE;    //幂等操作相关
    ...
    for (RegionAction regionAction : request.getRegionActionList()) {   //首先遍历request中的regionAction
      this.requestCount.add(regionAction.getActionCount());
      OperationQuota quota;
      Region region;
      regionActionResultBuilder.clear();
      try {
        region = getRegion(regionAction.getRegion());                   //获取对应的HRegion
    ...
    // Load the controller with the Cells to return. 如果需要返回数据的话就new一个createCellScanner扔回去
  ...
  throws ServiceException {
    ...
    try {
      ...
      requestCount.increment();         //scan被算作1次请求
            ...
            try {
              ...
              synchronized(scanner) {
                ...
                while (i < rows) {
                  ...
                  moreRows = scanner.nextRaw(values, scannerContext);   //这里是服务端scan真正获得数据的地方,这一句非常关键非常关键啊
