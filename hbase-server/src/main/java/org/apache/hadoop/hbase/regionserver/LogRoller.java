...
@VisibleForTesting
public class LogRoller extends HasThread {                     //注意这个程序内是含锁的
  ...
  public LogRoller(final Server server, final RegionServerServices services) {
    ...
    this.rollperiod = this.server.getConfiguration().
      getLong("hbase.regionserver.logroll.period", 3600000);    //rollperiod指定了每隔多少时间便产生一个Hlog
  ...
  public void run() {
    while (!server.isStopped()) {
      ...
      rollLock.lock(); // FindBugs UL_UNRELEASED_LOCK_EXCEPTION_PATH      锁在这里
      try {
