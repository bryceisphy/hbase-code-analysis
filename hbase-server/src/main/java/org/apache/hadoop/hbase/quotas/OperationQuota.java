...
/**
 * Interface that allows to check the quota available for an operation.    //用于检查每一个操作的可用限额
 */
...
public interface OperationQuota {
  ...
  /**
   * Keeps track of the average data size of operations like get, scan, mutate  //额度包含两个维度,sizeSum和count
   */
  public class AvgOperationSize {
    ...
    public AvgOperationSize() {
      int size = OperationType.values().length;  //values()方法返回枚举对象数组OperationQuota[],本例中length返回3
      sizeSum = new long[size];
      count = new long[size];
      for (int i = 0; i < size; ++i) {      //分别定义了三种操作的定额
        sizeSum[i] = 0;
        count[i] = 0;
      }
    }
    ...
    public void addOperationSize(OperationType type, long size) {
      if (size > 0) {
        int index = type.ordinal();         //ordinal()返回枚举对象的序数
        sizeSum[index] += size;             //把result的所有大小加上了
        count[index]++;                     //一次操作加一个
      }
    }
   ...
   */
  void checkQuota(int numWrites, int numReads, int numScans) throws ThrottlingException;   //根据限额和已用量检查是否可以执行指定操作;
