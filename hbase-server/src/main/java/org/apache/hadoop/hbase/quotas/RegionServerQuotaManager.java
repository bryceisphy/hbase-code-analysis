 ...
 * 
 * RegionServer在执行具体的(put/get/scan)前会从这个函数获得table/user的配额
 */
...
public class RegionServerQuotaManager {
  ...
  public void stop() {
    if (isQuotaEnabled()) {
      quotaCache.stop("shutdown");    //开启和关闭都是通过quotaCache实现的
    }
  }

  public boolean isQuotaEnabled() {
    return quotaCache != null;        //通过判断quotaCache是否为空判断是否生效
  }
  ...
  /**
   * Returns the quota for an operation.                  //根据ugi和table信息获取配额信息
  ...
  private OperationQuota checkQuota(final Region region, final int numWrites, final int numReads,
      final int numScans) throws IOException, ThrottlingException {   //以上两个函数最终指向的是这个函数
    User user = RpcServer.getRequestUser();                    //获取发起请求的用户
    ...
    TableName table = region.getTableDesc().getTableName();    //获取表名
