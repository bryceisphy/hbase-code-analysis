...
public class KeyValueHeap extends NonReversedNonLazyKeyValueScanner
  ...
  protected PriorityQueue<KeyValueScanner> heap = null;  //应用KVScannerComparator对所有KeyValueScanner排序
   ...
   */
  protected KeyValueScanner current = null;   //KeyValueHeap的KeyValueScanner包含了各种可能的Scanner
  ...
  KeyValueHeap(List<? extends KeyValueScanner> scanners,
    ...
    if (!scanners.isEmpty()) {
      this.heap = new PriorityQueue<KeyValueScanner>(scanners.size(),
          this.comparator);   //根据传入的KeyValueScanner构造出一个优先级队列(内部实现就是堆)
      for (KeyValueScanner scanner : scanners) {
        if (scanner.peek() != null) {
          this.heap.add(scanner);   //将元素逐一加入堆中
        } else {
          scanner.close();
        }
      }
      this.current = pollRealKV();  //从堆顶pop出一个KeyValueScanner放入成员变量current,KeyValueHeap的peek()
    }                               //取堆顶操作返回的是current.peek(),也就是current这个KeyValueScanner的堆顶KeyValue
  }

  public Cell peek() {                //返回堆顶,堆不变
    if (this.current == null) {
      return null;
    }
    return this.current.peek();  //实际上返回了第一个KeyValueScanner的第一个cell
  }

  public Cell next()  throws IOException {      //返回堆顶,堆顶出堆
    if(this.current == null) {
      return null;
    }
    ...
    if (kvNext == null) {
      this.current.close();
      this.current = null;          //一般调用pollRealKV前都要先执行这一句 HBase-13835
  ...
  public boolean next(List<Cell> result, ScannerContext scannerContext) throws IOException {
    ...
    InternalScanner currentAsInternal = (InternalScanner)this.current;   //把当前的KeyValueScanner作为InternalScanner对象
    boolean moreCells = currentAsInternal.next(result, scannerContext);  //调用InternalScanner对象的next方法
    Cell pee = this.current.peek();   //取当前KeyValueScanner的第一个kv
    ...
    if (pee == null || !moreCells) {
      this.current.close();   //如果后面没有值了可以关闭这个Scanner
    } else {
      this.heap.add(this.current);  //否则把这个Scanner放回heap,等待下一次调用
    }
    this.current = null;
    this.current = pollRealKV();   //然后再取一个新的Scanner
    if (this.current == null) {
      moreCells = scannerContext.setScannerState(NextState.NO_MORE_VALUES).hasMoreValues();  //如果再没有新的scanner
    }
    return moreCells;
  }
  ...
  private boolean generalizedSeek(boolean isLazy, Cell seekKey,
    ...
    while ((scanner = heap.poll()) != null) {
      ...
      if (isLazy && heap.size() > 0) {
        ...
        seekResult = scanner.requestSeek(seekKey, forward, useBloom);   //lazy seek优化
      } else {
   ...
   */
  protected KeyValueScanner pollRealKV() throws IOException {   //current要么是storefilescanner要么是memstore
    KeyValueScanner kvScanner = heap.poll();                    //current的选择由该函数实现
    if (kvScanner == null) {
      return null;
    }

    while (kvScanner != null && !kvScanner.realSeekDone()) {   //由realSeekDone可见,考虑到lazy seek优化
      if (kvScanner.peek() != null) {
        try {
          kvScanner.enforceSeek();   //HBase-13835
        ...
        if (curKV != null) {
          ...
          Cell nextKV = nextEarliestScanner.peek();  //取下一个sanner的kv也出来比较
