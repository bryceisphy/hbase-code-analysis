...
public class StoreScanner extends NonReversedNonLazyKeyValueScanner
    implements KeyValueScanner, InternalScanner, ChangedReadersObserver {   //调用StoreScanner的时候在初始化时候就完成了seek,然后调用next获得一个个结果
   ...
   * timeout checks.     //每scan出这些cells进行一次超时检查
   */
  ...
  // A flag whether use pread for scan     small scan使用pread而big scan使用seek+read(scan region在64KB内被认为是small scan) HBase-9488
  ...
  protected StoreScanner(Store store, boolean cacheBlocks, Scan scan,
    ...
    useRowColBloom = numCol > 1 || (!isGet && numCol == 1);     //是否执行row-column Bloom filters
  ...
  protected void seekScanners(List<? extends KeyValueScanner> scanners,
    ...
    if (isLazy) {
      ...
      if (!isParallelSeek) {            //这里根据isParallelSeek是否为真决定是否并行seek
  ...
  public boolean seek(Cell key) throws IOException {
    ...
    checkReseek();        //seek前先检查该store下的hife集合是否发生改变
  ...
  /**
   * Get the next row of values from this Store.   所有最终的取值操作都是落在了这里
   * @param outResult
  ...
  public boolean next(List<Cell> outResult, ScannerContext scannerContext) throws IOException {
    ...
    if (checkReseek()) {              //先检查store下的HFile集合是否发生改变
  ...
  public void updateReaders() throws IOException {
    ...
    // this could be null.
    this.lastTop = this.peek();       //将堆顶保存在类成员变量lastTop中
  ...
  protected boolean checkReseek() throws IOException {
    if (this.heap == null && this.lastTop != null) {   //this.heap为null说明当前Store下的HFile集合发生改变
      resetScannerStack(this.lastTop);      //将当前Store下的所有StoreFileScanner／MemStoreScanner都seek到lastTop
  ...
  public boolean reseek(Cell kv) throws IOException {
    ...
    checkReseek();                  //检查store下面的HFile集合是否发生改变
    if (explicitColumnQuery && lazySeekEnabledGlobally) {
      return heap.requestSeek(kv, true, useRowColBloom);   //lazy seek优化,该请求实际是发给了KeyValueHeap
    }
    return heap.reseek(kv);   //调用了KeyValueHeap的generalizedSeek
    } finally {
      lock.unlock();
    }
  }
  ...
  private void parallelSeek(final List<? extends KeyValueScanner>
      scanners, final Cell kv) throws IOException {               //并发seek storefiles
    if (scanners.isEmpty()) return;
    int storeFileScannerCount = scanners.size();
    CountDownLatch latch = new CountDownLatch(storeFileScannerCount);  //CountDownLatch是用AbstractQueuedSynchronizer实现的并发控制
    List<ParallelSeekHandler> handlers = 
        new ArrayList<ParallelSeekHandler>(storeFileScannerCount);
    for (KeyValueScanner scanner : scanners) {
      if (scanner instanceof StoreFileScanner) {    //只有StoreFileScanner才可以并行scan
        ParallelSeekHandler seekHandler = new ParallelSeekHandler(scanner, kv,
          this.readPt, latch);
        executor.submit(seekHandler);  //这一步是递交执行seekHandler里定义的process方法,process实际是执行了KeyValueScanner的seek方法
