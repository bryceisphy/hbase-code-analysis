...
public class TimeBasedLimiter implements QuotaLimiter {
  ...
  public void update(final TimeBasedLimiter other) {    //更新配额
  ...
  @Override
  public void checkQuota(long writeSize, long readSize) throws ThrottlingException {   //用于查看读写配额是否足够
  ...
  @Override
  public void grabQuota(long writeSize, long readSize) {    //消费writeSize+readSize的配额
    assert writeSize != 0 || readSize != 0;

    reqsLimiter.consume(1);
    reqSizeLimiter.consume(writeSize + readSize);     //配额这里都是先从reqSizeLimiter处理,然后依次writeSizeLimiter&readSizeLimiter
