...
public class MultiVersionConsistencyControl {
  ...
  public WriteEntry beginMemstoreInsertWithSeqNum(long curSeqNum) {
    WriteEntry e = new WriteEntry(curSeqNum);    //获得一个新的写序列号
    synchronized (writeQueue) {
      writeQueue.add(e);
      return e;
    }
  ...
  boolean advanceMemstore(WriteEntry e) {
    ...
    synchronized (writeQueue) {
      ...
      while (!writeQueue.isEmpty()) {
        ...
        if (queueFirst.isCompleted()) {           //庞皓元注:roll forward completed transactions.Ordering is perserved
      ...
      if (nextReadValue > memstoreRead) {
        memstoreRead = nextReadValue;     //向前推进了memstoreRead,事务完成,对读可见
      }
