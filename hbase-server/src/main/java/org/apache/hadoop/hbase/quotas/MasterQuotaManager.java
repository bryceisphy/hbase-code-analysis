...
public class MasterQuotaManager implements RegionStateListener {
  ...
  /*
   * ========================================================================== Admin operations to
   * manage the quota table    //用于管理quota table的相关DML操作
   */
  public SetQuotaResponse setQuota(final SetQuotaRequest req) throws IOException,               
      InterruptedException {                  //设置配额,注意这里对三个lock的使用姿势
    checkQuotaSupport();
  ...
  public void setUserQuota(final String userName, final SetQuotaRequest req) throws IOException,
    ...
    setQuota(req, new SetQuotaOperations() {         //setQuota的这种实现方式难以理解？？？？？？？
      @Override
  ...
  private void applyBypassGlobals(final Quotas.Builder quotas, boolean bypassGlobals) {   //Bypass是什么
    if (bypassGlobals) {
  ...
  private void createQuotaTable() throws IOException {
    ...
    if (masterServices.isMasterProcedureExecutorEnabled()) {
      masterServices.getMasterProcedureExecutor()              //由MasterProcedure产生quotaTable
        .submitProcedure(new CreateTableProcedure(
          masterServices.getMasterProcedureExecutor().getEnvironment(),
          QuotaUtil.QUOTA_TABLE_DESC,
          newRegions));
    } else {
      masterServices.getExecutorService().submit(              //直接由masterServices产生quotaTable
  ...
  private static class NamedLock<T> {   
    ...
    public void lock(final T name) throws InterruptedException {
      synchronized (locks) {     //锁住locks，只有获得locks才能访问这块代码块
