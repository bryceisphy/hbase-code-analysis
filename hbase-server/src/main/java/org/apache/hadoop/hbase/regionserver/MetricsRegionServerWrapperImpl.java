...
class MetricsRegionServerWrapperImpl
  ...
  private final HRegionServer regionServer;                       //regionServer实例

  private BlockCache blockCache;                                  //blockCache实例
  ...
  private ScheduledExecutorService executor;    //ScheduledExecutorService扩展了ExecutorService接口,提供时间排期功能
  ...
  public MetricsRegionServerWrapperImpl(final HRegionServer regionServer) {
    ...
    this.executor = CompatibilitySingletonFactory.getInstance(MetricsExecutor.class).getExecutor();   //从类MetricsExecutor中获得它的一个单实例,并返回其ScheduledExecutorService
    ...
    this.executor.scheduleWithFixedDelay(this.runnable, this.period, this.period,
      TimeUnit.MILLISECONDS);             //制定进程排期,使得runnable定期执行
  ...
  public class RegionServerMetricsWrapperRunnable implements Runnable {
    ...
    @Override
    synchronized public void run() {          //这个周期执行的run进程会收集所有regionserver和blockcache上的统计信息
      initBlockCache();                       //以更新这个类中的统计量
      ...
      for (Region r : regionServer.getOnlineRegionsLocalContext()) {
        ...
        tempReadRequestsCount += r.getReadRequestsCount();    //返回每个region上的读写请求数,做个汇总就是regionserver上的读写请求数
        tempWriteRequestsCount += r.getWriteRequestsCount();  //写请求的处理情况类似
