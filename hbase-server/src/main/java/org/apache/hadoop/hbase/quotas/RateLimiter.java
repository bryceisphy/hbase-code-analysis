...
public abstract class RateLimiter {
  ...
  private long tunit = 1000;           // Timeunit factor for translating to ms.                      时间单位
  private long limit = Long.MAX_VALUE; // The max value available resource units can be refilled to.  最大可用的资源数
  private long avail = Long.MAX_VALUE; // Currently available resource units                          目前有效的资源数
  ...
  /**
   * Time in milliseconds to wait for before requesting to consume 'amount' resource.    返回空出amount份资源前需要等待的时间
   * @param limit Maximum available resource units that can be refilled to.
  ...
  /**
   * Set the RateLimiter max available resources and refill period.                      设置RateLimiter的最大资源数和更新周期
  ...
  public synchronized boolean isBypass() {       //Bypass是通过将limit设为MAX_VALUE实现的
  ...
  /**
   * Is there at least one resource available to allow execution?     //至少有一个资源可用时canExecute会返回真
  ...
  /**
   * Are there enough available resources to allow execution?         //判断是否有amount份资源可用
   ...
   */
  public synchronized void consume(final long amount) {    //消耗amount份资源
    this.avail -= amount;
    if (this.avail < 0) {
      this.avail = 0;
    }
   ...
   */
  public long waitInterval() {     //在空出一份可消耗资源前需等待的时间
   ...
   * @return estimate of the ms required to wait before being able to provide "amount" resources.
   */
  public synchronized long waitInterval(final long amount) {    //在空出amount份可消耗资源前需等待的时间
