...
public class DefaultMemStore implements MemStore {    //管理memstore及其snapshot相关的类
  ...
  // reference passed.
  volatile CellSkipListSet cellSet;    //该memstore的cell集合,注意cell在内存中是以skip list组织

  // Snapshot of memstore.  Made for flusher.
  volatile CellSkipListSet snapshot;   //该memstore的snapshot集合
  ...
  public void rollback(Cell cell) {
    ...
    Cell found = this.snapshot.get(cell);       //先从snapshot中去除相关的cell信息
    ...
    found = this.cellSet.get(cell);             //然后再从cell set中去除相关的cell
    if (found != null && found.getSequenceId() == cell.getSequenceId()) {
