...
class MemStoreFlusher implements FlushRequester {
  ...
  public MemStoreFlusher(final Configuration conf,
    ...
    this.globalMemStoreLimit = (long) (max * globalMemStorePercent);        //控制全局memstore flush的变量
  ...
  private boolean flushOneForGlobalPressure() {
    ...
    while (!flushedOne) {
      ...
      Region bestAnyRegion = getBiggestMemstoreRegion(
          regionsBySize, excludedRegions, false);   //这两个getBiggestMemstoreRegion的区别在于是否考虑到已存的storefile数量
      ...
      if (bestFlushableRegion != null &&
        ...
        regionToFlush = bestAnyRegion;                //如果bestAnyRegion的memstore过大,将它作为备选
      } else {
        if (bestFlushableRegion == null) {
          regionToFlush = bestAnyRegion;
        } else {
          regionToFlush = bestFlushableRegion;        //否则选取bestFlushableRegion
        }
      }                         ////寻找出最适合flush的region
      ...
      if (regionToFlush == null ||
        ...
        flushedOne = refreshStoreFilesAndReclaimMemory(bestRegionReplica);    //刷新了一下这个region下面的storefile
        if (!flushedOne) {
        ...
        flushedOne = flushRegion(regionToFlush, true, true);        //memstore flush
  ...
  private class FlushHandler extends HasThread {          //flush线程

    private FlushHandler(String name) {
      super(name);
    }    //该线程会检查memstore的大小,如果其超过了额定值,会进行一次刷写
                                                          //该值由hbase.regionserver.global.memstore.size.lower.limit定义
    @Override
    public void run() {
        ...
        try {
          ...
          if (fqe == null || fqe instanceof WakeupFlushThread) {
            if (isAboveLowWaterMark()) {
              ...
              if (!flushOneForGlobalPressure()) {                                 //刷写函数隐藏在这里
    ...
    synchronized (regionsInQueue) {
      for (Region region : regionsBySize.values()) {
        ...
        if (((HRegion)region).writestate.flushing ||
            !((HRegion)region).writestate.writesEnabled) {      //排除掉正在flushing或者不可写的region
          continue;
        }

        if (checkStoreFileCount && isTooManyStoreFiles(region)) {     //第三个参数为真且该region含有许多storefiles时不会返回该region
          continue;
        }
        return region;
      }
  ...
  private boolean refreshStoreFilesAndReclaimMemory(Region region) {    //这个其实没有做flush,只是整理了一些从未打开的storefile
    try {
  ...
  private boolean flushRegion(final FlushRegionEntry fqe) {
    ...
    if (!region.getRegionInfo().isMetaRegion() &&
        isTooManyStoreFiles(region)) {                        //如果指定的region含有过多的storefiles
      if (fqe.isMaximumWait(this.blockingWaitTime)) {         //如果该region的等待时间已经超过了HRegion的阻塞更新时间
  ...
  private boolean flushRegion(final Region region, final boolean emergencyFlush,
    ...
    lock.readLock().lock();               //flush时该region的读是被锁住的
    try {
      ...
      if (shouldSplit) {
        this.server.compactSplitThread.requestSplit(region);        //region split的真正实现是在这里
      } else if (shouldCompact) {
        server.compactSplitThread.requestSystemCompaction(
            region, Thread.currentThread().getName());              //这里居然还有compact
      }
  ...
  private void wakeUpIfBlocking() {
    synchronized (blockSignal) {
      blockSignal.notifyAll();            //唤醒所有等待blockSignal的线程
    }
  }
  ...
  interface FlushQueueEntry extends Delayed {   //java接口Delayed,该接口定义了方法getDelay()
  }
  ...
  static class FlushRegionEntry implements FlushQueueEntry {
    private final Region region;          //请求刷写的region

    private final long createTime;        //用于计算FlushRegionEntry在队列中存在的时间
    private long whenToExpire;
    private int requeueCount = 0;         //这个应该是重试次数

    private boolean forceFlushAllStores;  //是否需要flush所有的stores
     ...
     * @return This.
     */
    public FlushRegionEntry requeue(final long when) {                  //这是重试了一次
