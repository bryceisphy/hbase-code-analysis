...
public class ClientScanner extends AbstractClientScanner {
    ...
    protected Scan scan;                                //保存扫描的常用参数
    protected boolean closed = false;                   //标志该scan是否已经关闭
    // Current region scanner is against.  Gets cleared if current region goes
    // wonky: e.g. if it splits on us.
    protected HRegionInfo currentRegion = null;         //当前region信息
    protected ScannerCallableWithReplicas callable = null;
    protected final LinkedList<Result> cache = new LinkedList<Result>();        //缓存数量为caching的Result
    /**
     ...
     * result.
     */
    protected final LinkedList<Result> partialResults = new LinkedList<Result>();  //用于接收server返回的部分结果
    /**
    ...
    protected final int caching;                    //一次查询缓存的数量
    protected long lastNext;                        //最后一次查询的时间
    // Keep lastResult returned successfully in case we have to reset scanner.
    protected Result lastResult = null;          //上一次查询的最后一个结果,方便再次启动查询
    protected final long maxScannerResultSize;   //最大查询结果的size
    private final ClusterConnection connection;
    private final TableName tableName;
    protected final int scannerTimeout;          //scan的超时时间
    ...
    protected boolean checkScanStopRow(final byte [] endKey) {
      if (this.scan.getStopRow().length > 0) {
        ...
        if (cmp <= 0) {
          ...
          // This is a stop.
          return true;        //已经比LAST_ROW还大了,返回true,停止scan
        }
      }
  ...
  protected boolean nextScanner(int nbRows, final boolean done)
    throws IOException {
      // Close the previous scanner if it's open  关闭可能还打开的scanner
      ...
      if (this.currentRegion != null) {
        ...
        if (endKey == null ||
          ...
          close();             //已经scan至表尾则关闭scan并返回false给调用者
          if (LOG.isTraceEnabled()) {
      ...
      try {
        callable = getScannerCallable(localStartKey, nbRows);   //获得一个可调用程序,该程序执行时会从regionserver或其replica中取得数据
        // Open a scanner on the region server starting at the
        // beginning of the region
        call(callable, caller, scannerTimeout);     //向regionserver发起rpc请求,call only once
  ...
  Result[] call(ScannerCallableWithReplicas callable,
    ...
    return caller.callWithoutRetries(callable, scannerTimeout);  //callWithoutRetries是指call the server只一次
  }
    ...
    protected ScannerCallableWithReplicas getScannerCallable(byte [] localStartKey,
      ...
      ScannerCallableWithReplicas sr = new ScannerCallableWithReplicas(tableName, getConnection(),
       ...
       retries, scannerTimeout, caching, conf, caller);   //这是一个带replicas的scanner
      return sr;
    }
     ...
     */
    protected void writeScanMetrics() {     //这里有个metrics
    ...
    public Result next() throws IOException {
      ...
      if (cache.size() == 0) {
        loadCache();             //当cache为空的时候，从regionserver中加载一部分Results到cache中
      }

      if (cache.size() > 0) {
        return cache.poll();     //否则直接从cache取一份Result返回
      }

      // if we exhausted this scanner before calling close, write out the scan metrics
      writeScanMetrics();           //metrics统计结果是调用这个函数写入的
      return null;
    }
   ...
   */
  protected void loadCache() throws IOException {       //从server中load数据到cache
    Result[] values = null;
    ...
    callable.setCaching(this.caching);                                  //设置cache的大小
    ...
    boolean serverHasMoreResults = false;                               //是否还有更多的数据
    do {
      try {
        // Server returns a null values if scanning is to stop. Else,
        // returns an empty array if scanning is to go on and we've just
        // exhausted current region.
        values = call(callable, caller, scannerTimeout);     //开始向regionserver拿数据
        ...
        if (values == null && callable.switchedToADifferentReplica()) {   //若数据拿取失败且切换到了新的replicaRS
          ...
          clearPartialResults();
          this.currentRegion = callable.getHRegionInfo();   //获取新的region信息
          continue;
        }
        ...
        if (this.lastResult != null) {
          ...
          if (scan.isReversed()) {
            scan.setStartRow(createClosestRowBefore(lastResult.getRow()));   //重新设置start row
          } else {
            scan.setStartRow(Bytes.add(lastResult.getRow(), new byte[1]));
          }
        }
        if (e instanceof OutOfOrderScannerNextException) {              //HBase-7070
          if (retryAfterOutOfOrderException) {
            retryAfterOutOfOrderException = false;      //看到OutOfOrderScannerNextException异常时,客户端不再重试
          } else {
      ...
      if (this.scanMetrics != null) {
        this.scanMetrics.sumOfMillisSecBetweenNexts.addAndGet(currentTime - lastNext);  //Metrics是这么加入的
      }
      ...
      if (null != values && values.length > 0 && callable.hasMoreResultsContext()) {
        ...
        serverHasMoreResults = callable.getServerHasMoreResults() & partialResults.isEmpty();   //该标志为true时表示该region还有数据
      }
  ...
  private void addResultsToList(List<Result> outputList, Result[] inputArray, int start, int end) {
    ...
    @Override
    public void close() {
      if (!scanMetricsPublished) writeScanMetrics();  //scan关闭的时候会把metrics统计结果上报
      if (callable != null) {
        callable.setClose();
        try {
          call(callable, caller, scannerTimeout);
