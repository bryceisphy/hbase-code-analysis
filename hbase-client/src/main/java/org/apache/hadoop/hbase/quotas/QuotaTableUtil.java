...
public class QuotaTableUtil {
  ...
  private static Quotas getQuotas(final Connection connection, final byte[] rowKey,
      final byte[] qualifier) throws IOException {      //至此getQuotas是从quotas table中读出相关Quota的类
   ...
   */
  public static Filter makeFilter(final QuotaFilter filter) {        //将对象QuotaFilter转变为FilterList
  ...
  public static void parseResult(final Result result, final QuotasVisitor visitor)   
      throws IOException {                                 //解析result相关结果的一个类
  ...
  public static void
      parseNamespaceResult(final Result result, final NamespaceQuotasVisitor visitor)
          throws IOException {        //这些parse类的作用是从读table的结果解析出Quotas,然后由该Quotas构建vistor
  ...
  /*
   * ========================================================================= Quota table row key
   * helpers   各种与Quota table的row key相关的类
   */
