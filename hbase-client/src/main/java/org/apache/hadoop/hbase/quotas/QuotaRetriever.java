...
/**
 * Scanner to iterate over the quota settings.    这个类的实现是不是就是对quota table的一次scan
 */
...
public final class QuotaRetriever implements Closeable, Iterable<QuotaSettings> {
  ...
  private final Queue<QuotaSettings> cache = new LinkedList<QuotaSettings>();   //用于存储QuotaRetriever的结果
  ...
  void init(final Configuration conf, final Scan scan) throws IOException {    //初始化获得quota table
    this.connection = ConnectionFactory.createConnection(conf);
    this.table = this.connection.getTable(QuotaTableUtil.QUOTA_TABLE_NAME);
    try {
      scanner = table.getScanner(scan);         //这一步获得了scanner
    } catch (IOException e) {
      try {
        close();
  ...
  public QuotaSettings next() throws IOException {    //scanner向下走一步获得下一个结果
    if (cache.isEmpty()) {
      Result result = scanner.next();
      if (result == null) return null;

      QuotaTableUtil.parseResult(result, new QuotaTableUtil.QuotasVisitor() {   //将读出的结果用这种方式处理
        @Override
  ...
  private class Iter implements Iterator<QuotaSettings> {    //用定义的内部类实现了迭代器
  ...
  public static QuotaRetriever open(final Configuration conf, final QuotaFilter filter)   
      throws IOException {                              //使用指定的QuotaFilter打开一个QuotaRetriever
