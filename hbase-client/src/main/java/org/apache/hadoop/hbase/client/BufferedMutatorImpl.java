...
public class BufferedMutatorImpl implements BufferedMutator {
  ...
  private List<Row> writeAsyncBuffer = new LinkedList<>();   //需要提交的数据列表
  private long writeBufferSize;                              //flush的size
  private final int maxKeyValueSize;
  protected long currentWriteBufferSize = 0;                 //当前数据的size,达到指定size就要提交
  private boolean closed = false;
  private final ExecutorService pool;                        //For multi
  ...
  public synchronized void mutate(Mutation m) throws InterruptedIOException,
      RetriesExhaustedWithDetailsException {          //执行单独的Mutation操作
    doMutate(m);
  }

  @Override
  public synchronized void mutate(List<? extends Mutation> ms) throws InterruptedIOException,
      RetriesExhaustedWithDetailsException {          //执行批量操作
    for (Mutation m : ms) {
      doMutate(m);
    }
  }
  ...
  private void doMutate(Mutation m) throws InterruptedIOException,
    ...
    if (ap.hasError()) {                //异步提交出错,执行后台提交
      writeAsyncBuffer.add(m);
      backgroundFlushCommits(true);     //注意这次是同步的
    }

    if (m instanceof Put) {
      validatePut((Put) m);    //如果m是put操作,检测是否含有非法put
    }

    currentWriteBufferSize += m.heapSize();
    writeAsyncBuffer.add(m);           //否则将m添加到当前的执行队列中

    while (currentWriteBufferSize > writeBufferSize) {  //当buffer超过我们的设置值时就flush掉
  ...
  private void backgroundFlushCommits(boolean synchronous) throws InterruptedIOException,
    ...
    try {
      ...
      if (synchronous || ap.hasError()) {                                   //同步后台提交
