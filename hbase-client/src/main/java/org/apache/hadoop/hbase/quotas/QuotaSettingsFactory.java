...
public class QuotaSettingsFactory {
   ...
   * the Quotas object
   */
  static List<QuotaSettings> fromUserQuotas(final String userName, final Quotas quotas) {    //从Quotas object中解析出QuotaSettings并存放在List<QuotaSettings>中
  ...
  private static QuotaSettings throttle(final String userName, final TableName tableName,
    ...
    QuotaProtos.ThrottleRequest.Builder builder = QuotaProtos.ThrottleRequest.newBuilder();   //构建builder,然后把builder传入ThrottleSettings这种方式
    if (type != null) {
