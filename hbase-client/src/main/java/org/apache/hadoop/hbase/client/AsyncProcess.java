...
class AsyncProcess {        //异步提交RPC
  ...
  public <CResult> AsyncRequestFuture submit(ExecutorService pool, TableName tableName,
    ...
    do {
      ...
      int posInList = -1;
      Iterator<? extends Row> it = rows.iterator();    //循环遍历rows
      while (it.hasNext()) {
        Row r = it.next();
        HRegionLocation loc;
        try {
          ...
          RegionLocations locs = connection.locateRegion(                               //为row进行定位
        ...
        if (canTakeOperation(loc, regionIncluded, serverIncluded)) {
          ...
          addAction(loc.getServerName(), regionName, action, actionsByServer, nonceGroup);   //把loc和action一起添加到actionsByServer
          it.remove();
        }
      }
    ...
    return submitMultiActions(tableName, retainedActions, nonceGroup, callback, null, needResults,
      locationErrors, locationErrorRows, actionsByServer, pool);   //提交动作
  }
  ...
  <CResult> AsyncRequestFuture submitMultiActions(TableName tableName,
    ...
    ars.sendMultiAction(actionsByServer, 1, null, false);   //异步请求的实现
    return ars;
  }
   ...
   */
  protected class AsyncRequestFutureImpl<CResult> implements AsyncRequestFuture {     //内部类AsyncRequestFutureImpl
    ...
    private final class SingleServerRequestRunnable implements Runnable {
      ...
      public void run() {
        ...
        try {
          callable = createCallable(server, tableName, multiAction);     //多线程rpc提交
          try {
  ...
  protected MultiServerCallable<Row> createCallable(final ServerName server,
    ...
    return new MultiServerCallable<Row>(connection, tableName, server, this.rpcFactory, multi);  //构造一个MultiServerCallable对象
  }
  ...
  protected RpcRetryingCaller<MultiResponse> createCaller(MultiServerCallable<Row> callable) {
    return rpcCallerFactory.<MultiResponse> newCaller();             //最后通过rpcCallerFactory执行call操作;
  }
