...
public class ClientSmallScanner extends ClientScanner {                 //HBase-9488应用于small scanner
    ...
    if (this.currentRegion != null && currentRegionDone) {
      ...
      if (endKey == null || Bytes.equals(endKey, HConstants.EMPTY_BYTE_ARRAY)
        ...
        if (LOG.isTraceEnabled()) {
          LOG.trace("Finished with small scan at " + this.currentRegion);   //small scan已经全局结束
        }
        return false;
      }
      localStartKey = endKey;
      if (LOG.isTraceEnabled()) {
        LOG.trace("Finished with region " + this.currentRegion);    //当前region的small scan已经结束
      }
