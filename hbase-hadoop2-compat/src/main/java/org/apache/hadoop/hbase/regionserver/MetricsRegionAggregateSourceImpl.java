...
public class MetricsRegionAggregateSourceImpl extends BaseSourceImpl
  ...
  @Override
  public void register(MetricsRegionSource source) {        //source定义了信息源
    lock.writeLock().lock();
    try {
  ...
  @Override
  public void deregister(MetricsRegionSource source) {    //通过上面的两个函数可以动态地增加或者删除信息源
    lock.writeLock().lock();
    try {
  ...
  public void getMetrics(MetricsCollector collector, boolean all) {
    ...
    metricsRegistry.snapshot(mrb, all);   //动态的添加/删除信息源
  }
}
