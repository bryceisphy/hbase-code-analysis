...
. "$bin"/hbase-config.sh    ##装载各种配置
. "$bin"/hbase-common.sh

# get arguments
startStop=$1                ##把命令输入的第一个参数记下
shift
...
export HBASE_LOG_PREFIX=hbase-$HBASE_IDENT_STRING-$command-$HOSTNAME            ##这两句指定了log文件的格式
export HBASE_LOGFILE=$HBASE_LOG_PREFIX.log

if [ -z "${HBASE_ROOT_LOGGER}" ]; then
export HBASE_ROOT_LOGGER=${HBASE_ROOT_LOGGER:-"INFO,RFA"}                 ##RFA中定义HBase log的配置(HBASE-5655)
fi

if [ -z "${HBASE_SECURITY_LOGGER}" ]; then 
export HBASE_SECURITY_LOGGER=${HBASE_SECURITY_LOGGER:-"INFO,RFAS"}        ##RFAS中定义HBase Security log的配置(HBASE-6010)
fi
...
thiscmd="$bin/$(basename ${BASH_SOURCE-$0})"        ##表示该hbase-dameon.sh文件的绝对路径
args=$@

case $startStop in

(start)
    check_before_start                                 ##start前先检查进程是否已启动
    hbase_rotate_log $HBASE_LOGOUT                     ##回滚一下out日志和gc日志
